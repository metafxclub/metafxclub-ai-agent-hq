...
{
   string temporary_path = final_path + ".tmp";
   ResetLastError();
   int handle = FileOpen(
      temporary_path,
...
void PublishSnapshotIfDue(const bool force)
{
   ...
   if(g_last_snapshot_write_ok)
      ...
      Print("MetafxHQ: Unable to write snapshot.json.");
}
...
void RefreshManagedOutcomeFiles(const bool force)
{
      ...
      history_index--)
   {
      ...
      if(IsGatewayOrderComment(OrderComment(), history_command_id))
...
)
{
   ...
   bool slippage_within_limit = point > 0.0 &&
      g_ack_filled_slippage_points <= (double)SlippagePoints + 0.001;
   bool matches = OrderTicket() == ticket &&
      ...
      StringFind(OrderComment(), expected_comment, 0) == 0 &&
