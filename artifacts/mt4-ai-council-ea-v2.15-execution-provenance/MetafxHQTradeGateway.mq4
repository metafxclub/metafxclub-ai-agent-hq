...
bool IsAllowedBrokerSymbol(const string csv, const string candidate)
{
   ...
   for(int index = 0; index < count; index++)
   {
      ...
      // broker suffix (for example EURUSD.m, XAUUSDpro or XAUUSD-ECN).
...
bool ReadLastOrderBar(int &bar_time)
{
   bar_time = 0;
   string raw = "";
   if(!ReadCommonText(LastOrderBarPath(), 64, raw))
      return true;
   raw = Trimmed(raw);
   if(!IsIntegerToken(raw))
      return false;
   bar_time = (int)StringToInteger(raw);
   return true;
}
...
bool ResolveSelectedOrderCommandId(string &command_id)
{
   if(ReadSelectedOrderTicketMap(command_id))
      return true;
   return IsGatewayOrderComment(OrderComment(), command_id);
}


bool WriteSelectedOrderOutcome(const string command_id)
{
   if(!IsSafeIdentifier(command_id) || !IsManagedMarketOrderSelected())
      return false;
...
)
{
   ...
   // Re-run every mutable guard at the final execution boundary. There is no
   ...
   // Recompute HMAC and compare the complete decoded command one final time at
   // the irreversible execution boundary. Demo and Live share this path.
   if(!ReverifyCommandEnvelope(signed_raw, command, reason))
   {
      FinalizeCommand(command, "REJECTED", reason, -1, 0);
      return;
   }
   // Claim the bar only after every pre-send validation has passed.  This
   // avoids consuming the whole bar because of a transient stale quote,
   // heartbeat, market-session or SL/TP rejection.  The claim is still
   // persisted before OrderSend, so a crash can never cause an automatic
   // resend on the same closed bar.
   ...
   // One final kill-switch and expiry read immediately before OrderSend. If
   // either changes after reserving the bar, the bar remains consumed
   // deliberately rather than risking a duplicate execution.
   if(FileIsExist(KillMarkerPath(), FILE_COMMON))
   {
      FinalizeCommand(command, "REJECTED", "KILL_SWITCH_ACTIVE", -1, 0);
      return;
   ...
   if(!WriteTicketCommandMap(command, ticket))
   {
      ...
      // OrderSend has already succeeded. Capture whatever immutable evidence
      // is selectable, but remain uncertain because future TP/SL closure could
      // not be bound back to the complete command ID without this map.
...
void ApplyOptionalPositionLifecycle()
{
   ...
   for(int index = OrdersTotal() - 1; index >= 0; index--)
   {
      ...
      bool holding_due = LifecycleUsesMaxHolding() && MaxHoldingMinutes > 0 &&
         now >= (int)OrderOpenTime() + MaxHoldingMinutes * 60;
