...
ข้อความนี้เป็นการยืนยันล่วงหน้าให้ Codex เลือก Local endpoint รายการแรกที่ตัวติดตั้งรายงานว่า available=true บน 127.0.0.1 และส่งค่านั้นกลับเข้า installer ด้วย -EndpointConfirmed ได้ ไม่ต้องถามฉันให้เลือก Port หรือกดไฟล์ BAT ซ้ำ ห้ามใช้ 0.0.0.0, LAN IP หรือ Public IP
...
6. จากโฟลเดอร์ Release ให้เรียก:
   powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File ".\installer\install.ps1" -ListAvailableEndpoints
   อ่าน JSON ที่ได้และเลือก candidates รายการแรกที่ available=true เท่านั้น หากพอร์ตถูกใช้ก่อนเริ่มติดตั้ง ให้ดึงรายการใหม่และเลือก available รายการแรกได้อีกไม่เกิน 3 ครั้ง ห้ามปิด Process อื่น

7. ติดตั้งด้วย Port ที่เลือกโดยตรง ห้ามใช้ -SkipLaunch:
   powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File ".\installer\install.ps1" -Port PORT_ที่เลือก -EndpointConfirmed -SkipGoogleSetup
   รอให้ตัวติดตั้งรัน Preflight, สร้าง pinned venv, รัน Tests, เปิด Bridge และตรวจ Health จบ ห้ามรายงานว่าสำเร็จถ้าตัวติดตั้งคืน Exit code ที่ไม่ใช่ 0

8. หลังติดตั้งสำเร็จ ให้นำเข้า OAuth JSON ผ่านสคริปต์ของ Runtime ที่ติดตั้งจริงเท่านั้น:
   powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "$env:LOCALAPPDATA\Metafxclub\AI-Agent-HQ\scripts\setup-google-oauth.ps1" -ClientJsonPath "<Path เต็มจาก GOOGLE_DESKTOP_OAUTH_JSON>" -SkipOpen
...
9. อ่าน url, health_url และ port จาก:
   ...
   แล้วตรวจ health_url ต้องได้ ok=true, status=ready, endpoint.host=127.0.0.1 และ endpoint.port ตรงกับ Port ที่เลือก
...
11. เรียก scripts/check-codex-readiness.cmd จากโฟลเดอร์ติดตั้งจริง แล้วเปิด url ที่อ่านจาก bridge-endpoint.json ห้ามเดา Port
...
14. สรุปท้ายงานเฉพาะ Version, ตำแหน่งติดตั้ง, URL, Health, สถานะ Codex/Rate Limit, สถานะ Google Client แบบปิดบัง และสิ่งที่ฉันต้องกดเอง ห้ามแสดง Client Secret, Refresh Token, Access Token, Cookie, Auth file หรือเนื้อหา JSON
