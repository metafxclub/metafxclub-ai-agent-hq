...
class RadarScheduleHardDailyCapTests(unittest.TestCase):
    """The Radar runtime, not merely its form, enforces one Bangkok run/day."""
    ...
    def test_once_daily_slot_does_not_dispatch_again_on_next_tick(self) -> None:
        calls: list[dict] = []
        ...
        self.assertTrue(first["dispatched"])
        self.assertEqual(first["slotKey"], "indicatorScoutSchedule:2026-08-12:0900")
        self.assertFalse(second["dispatched"])
        self.assertEqual(len(calls), 1)
    ...
    def test_contracts_declare_runtime_cap_restart_and_midnight_policies(self) -> None:
        ...
        self.assertEqual(schedule["countingPolicy"], "post_gate_pre_dispatch_fail_closed_reservation")
